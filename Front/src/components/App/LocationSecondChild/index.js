...
import LocationThirdChild from '../LocationThirdChild';

// == Composant
const LocationSecondChild = ({ id, label }) => {
    ...
    <div className="location">
      ...
      {childrenShown && (
        locationChildren.map((locationChild) => (
          <LocationThirdChild key={locationChild.id} {...locationChild} />
        ))
      )}
    </div>
  );
...
LocationSecondChild.propTypes = {
  id: PropTypes.number.isRequired,
  label: PropTypes.string.isRequired,
};

// == Export
export default LocationSecondChild;
